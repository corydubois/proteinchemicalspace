...
require(dplyr)
#set color palette
cbbPalette <- c("#000000", "#E69F00", "#0072B2","#F0E442", "#009E73", "#CC79A7")
```
...
rando15 <- fread("select100Klibrary_5mil_top1000.tsv",na.strings=NULL)
rando15<-rando15[,!c("pept_length"),with=FALSE]
rando15[is.na(rando15)]<-0

#load kmeans
kmeans <- fread("library_generatedby_kmeans.csv",na.strings=NULL)
kmeans<-kmeans[,!c("V1"),with=FALSE]
...
rando15[,pcomp1:=rando.pca15[,1]][,pcomp2:=rando.pca15[,2]][,PC1_bins:=76][,PC2_bins:=76][,factor_len:=101][,factor_var:="Extreme"]
kmeans[,pcomp1:=kmeans.pca[,1]][,pcomp2:=kmeans.pca[,2]][,PC1_bins:=76][,PC2_bins:=76][,factor_len:=101][,factor_var:="Kmeans"]
...
liberry <- rbind(liberry,dispersed,random,rando15,kmeans,knn)
...
pca <- ggplot(data=liberry.histo,aes(x=pcomp1 ,y=pcomp2 ,colour=factor_var))+geom_point(alpha=0.3,shape = 20, size = 0.2)+
  scale_colour_manual(values=cbbPalette[2:6])+
...
vio1 <- ggplot(data=liberry,aes(x=factor_var,y=pcomp1 ,fill=factor_var))+
  geom_violin(bw=1.5)+
  ...
  ylab("PC1 (37.8%)")
save_plot(filename="vio_1.png",vio1)

#code for violin plot of PC2
vio2 <- ggplot(data=liberry,aes(x=factor_var,y=pcomp2 ,fill=factor_var))+
...
save_plot(filename="vio_2.png",vio2)
...
outplot <- ggplot(data=liberry.histo,aes(x=pcomp1, fill=factor_var))+
  geom_histogram(alpha = 0.5,position="dodge",bins=25)
save_plot(filename="Histo_1.png",outplot)

#code for histogram of PC2
outplot <- ggplot(data=liberry.histo,aes(x=pcomp2, fill=factor_var))+
  geom_histogram(alpha = 0.5,position="dodge",bins=20)
save_plot(filename="Histo_2.png",outplot)
